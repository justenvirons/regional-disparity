...
``` {r import-subset-data, include=FALSE}
...
studyarea_tracts <- places_tracts %>% 
  select(-Geolocation) %>%
...
acs2019_bycounty <- acs2019_bytract %>% 
  mutate(GEOID_county = substr(GEOID,1,5)) %>%
  filter(GEOID_county=="17031" | GEOID_county=="17043" |
           GEOID_county=="17089" | GEOID_county=="17093" |
           GEOID_county=="17097" | GEOID_county=="17111" |
           GEOID_county=="17197" | GEOID_county=="18089") %>%
  group_by(GEOID_county) %>%
  summarize_at(.vars = vars(TotalPop:PopWht, TotalPov:PopBPov), .funs=c(sum="sum")) %>% 
  right_join(studyarea_counties, by=c("GEOID_county"="geoid_county")) %>%
  st_as_sf() %>%
  st_transform(crs=4326)

labels <- sprintf(
  "<strong>%s</strong><br/>%g mi<sup>2</sup>",
  acs2019_bycounty$name_county, acs2019_bycounty$sqmi_county) %>% lapply(htmltools::HTML)

leaflet() %>% 
  addPolygons(data=acs2019_bycounty,
              fillColor = "orange",
              weight = 2,
              opacity = 0.5,
              color = "white",
